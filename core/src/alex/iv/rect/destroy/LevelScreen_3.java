...
public class LevelScreen_3 extends LevelScreenMain {
    ...
    public void initialize() {
        ...
        quantityBricks(50, 5);
        showModalScreen(3, recordsLevel_3, "rules_level_screen/level_screen_3_ru.txt",
                "rules_level_screen/level_screen_3.txt");
    ...
    public void update(float dt) {
        ...
        if (starTimer < 0) {
            timeIsUp(recordsLevel_3, "records_3"); // инициализируем метод timeIsUp - ВРЕМЯ ВЫШЛО
            RectangleGame.setActiveScreen(new IntermediateScreen(requestHandler, 3, score));
        }
        if (BaseActor.count(mainStage, "alex.iv.rect.destroy.actors.Brick") == 0 && starTimer > 0) {
            allTheBricksAreBroken("records_3");// инициализируем метод allTheBricksAreBroken - ВСЕ КИРПИЧИ РАЗРУШЕНЫ
            RectangleGame.setActiveScreen(new VictoryScreen(requestHandler, "attainmentColorLevelMemory_3"));
        }
