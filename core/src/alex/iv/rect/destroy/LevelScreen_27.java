...
public class LevelScreen_27 extends LevelScreenMain {
    ...
    public void initialize() {
        ...
        showModalScreen(27, recordsLevel_27, "rules_level_screen/level_screen_27_ru.txt",
                "rules_level_screen/level_screen_27.txt");
        quantityBricks(76, 6);
    ...
    public void update(float dt) {
        ...
        if (starTimer < 0) {
            timeIsUp(recordsLevel_27, "records_27"); // инициализируем метод timeIsUp - ВРЕМЯ ВЫШЛО
            RectangleGame.setActiveScreen(new IntermediateScreen(requestHandler, 27, score));
        }
        if (BaseActor.count(mainStage, "alex.iv.rect.destroy.actors.Brick") == 0 && starTimer > 0) {
            allTheBricksAreBroken("records_27");// инициализируем метод allTheBricksAreBroken - ВСЕ КИРПИЧИ РАЗРУШЕНЫ
            RectangleGame.setActiveScreen(new VictoryScreen(requestHandler, "attainmentColorLevelMemory_27"));
        }
